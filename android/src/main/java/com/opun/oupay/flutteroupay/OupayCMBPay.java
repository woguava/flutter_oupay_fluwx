...
public class OupayCMBPay  implements PluginRegistry.ActivityResultListener{
    ...
    public void starPay(String appid, String payInfo, boolean isSandbox, MethodChannel.Result result) {
        ...
        request.method = "pay"; //默认值 pay
        request.requestData = payInfo; //透传签名数据
        request.CMBH5Url = CmbRegister.get_mH5Url(isSandbox); //h5 url
        request.CMBJumpAppUrl = CmbRegister.get_mCMBJumpUrl(isSandbox); //Jump url

        Log.d("OUPAY_CMB",request.CMBH5Url);
        Log.d("OUPAY_CMB",request.CMBJumpAppUrl);
